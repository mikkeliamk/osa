...
<xsl:stylesheet version="1.0"
		...
		xmlns:osa="http://www.mamk.fi"
		xmlns:c="http://www.mamk.fi"
        xmlns:m="http://www.mamk.fi"
	...
	<xsl:template match="/foxml:digitalObject" mode="activeFedoraObject">
		...
		<doc> 
			...
			<xsl:for-each select="foxml:datastream/foxml:datastreamVersion[last()]/foxml:xmlContent/oai_dc:dc/*">
				<field>
					<xsl:attribute name="name">
						<xsl:value-of select="concat('dc.', substring-after(name(),'dc:'))"/>
					</xsl:attribute>
			...
			<!-- Capture -->
			<xsl:for-each select="foxml:datastream/foxml:datastreamVersion[last()]/foxml:xmlContent/osa:capture/*">
				<xsl:choose>
					<xsl:when test="substring-after(name(),'c:')='agent' or substring-after(name(),'c:')='controlledsubject'">
						<xsl:for-each select="./*">
							<field>
								<xsl:attribute name="name">
									<xsl:value-of select="concat('c.',substring-after(name(),'c:'))"/>
								</xsl:attribute>
								<xsl:value-of select="text()"/>
							</field>
						</xsl:for-each>
			...
			<!-- Management -->
			<xsl:for-each select="foxml:datastream/foxml:datastreamVersion[last()]/foxml:xmlContent/osa:management/*">
				<field>
					<xsl:attribute name="name">
						<xsl:value-of select="concat('m.', substring-after(name(),'m:'))"/>
					</xsl:attribute>
					<xsl:value-of select="text()"/>
			    </field>
			</xsl:for-each>
			...
			<!-- Admin -->
			<!-- Not used anywhere but is kept in case it could be used sometime -->
			<!-- <xsl:choose>
				<xsl:when test="foxml:datastream/foxml:datastreamVersion[last()]/foxml:xmlContent/osa:capture/c:isAncestor='true'">
					<field>
						<xsl:attribute name="name">ancestor</xsl:attribute>
						<xsl:value-of select="$PID"/>
					</field>
				</xsl:when>
				<xsl:otherwise>
	...
	<!-- Delete inactive documents from Solr -->
